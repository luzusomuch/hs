...
const paths = {
  client: {
    ...
    styles: [`${clientPath}/{styles}/app/**/*.scss`],
    backendStyles: [`${clientPath}/{styles}/backend/**/*.scss`],
    mainStyle: `${clientPath}/app/app.scss`,
    backendStyle: `${clientPath}/backend/app.scss`,
    views: `${clientPath}/{app,components}/**/*.html`,
    backendViews: `${clientPath}/{app,components}/**/*.html`,
    mainView: `${clientPath}/index.html`,
    backendView: `${clientPath}/backend.html`,
...
gulp.task('inject:js:backend', () => {
  return gulp.src(paths.client.backendView)
  .pipe(plugins.inject(
      gulp.src(_.union(paths.client.scripts, [
        `!${clientPath}/**/*.{spec,mock}.js`,
        `!${clientPath}/app/**/*.js`,
        `!${clientPath}/backend/app.js`
      ]), {read: false})
        .pipe(plugins.sort(sortModulesFirst)),
      {
        starttag: '<!-- injector:js -->',
...
gulp.task('inject:scss:backend', () => {
  return gulp.src(paths.client.backendStyle)
    .pipe(plugins.inject(
      gulp.src(_.union(paths.client.backendStyles, ['!' + paths.client.backendStyle]), {read: false})
        .pipe(plugins.sort()),
      {
        transform: (filepath) => {
          let newPath = filepath
            .replace(`/${clientPath}/backend/`, '')
            .replace(`/${clientPath}/components/`, '../components/')
            .replace(/_(.*).scss/, (match, p1, offset, string) => p1)
            .replace('.scss', '');
          return `@import '${newPath}';`;
    ...
    .pipe(gulp.dest(`${clientPath}/backend`));
});
...
gulp.task('watch', () => {
  ...
  plugins.watch(paths.client.backendStyles, () => {  //['inject:scss']
    gulp.src(paths.client.backendStyle)
      .pipe(plugins.plumber())
      .pipe(styles())
      .pipe(gulp.dest('.tmp/backend'))
      .pipe(plugins.livereload());
...
gulp.task('build:client:backend', () => {
  ...
  var backendFilter = plugins.filter('**/backend/app.js', {restore: true});
  var jsFilter = plugins.filter('**/*.js', {restore: true});
  var cssFilter = plugins.filter('**/*.css', {restore: true});
  var htmlBlock = plugins.filter(['**/*.!(html)'], {restore: true});
  ...
  return gulp.src(paths.client.backendView)
    .pipe(plugins.useref())
      .pipe(backendFilter)
        .pipe(plugins.addSrc.append('.tmp/templatesBackend.js'))
        .pipe(plugins.concat('backend/app.js'))
      .pipe(backendFilter.restore)
      .pipe(jsFilter)
        .pipe(plugins.ngAnnotate())
        .pipe(plugins.uglify())
      .pipe(jsFilter.restore)
